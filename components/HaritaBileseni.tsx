...
function HaritaKontrol({ hedef }: { hedef: { lat: number, lng: number } | null }) {
  ...
  useEffect(() => {
    if (hedef) {
      map.flyTo([hedef.lat, hedef.lng], 16, { duration: 2 });
    }
  }, [hedef, map]);
  return null;
}
...
export default function HaritaBileseni() {
  ...
  useEffect(() => {
    const verileriGetir = async () => {
      ...
      if (data) setVincler(data.filter(v => v.lat && v.lng));
      setYukleniyor(false);
    };
    verileriGetir();
  }, []);
  ...
  if (yukleniyor) return <div className="h-full flex items-center justify-center"><Loader2 className="animate-spin w-10 h-10 text-blue-600"/></div>;
