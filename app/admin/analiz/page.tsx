...
// Versiyon: 6.0 (Personel Performans Takibi + Akıllı Seçim)
// ----------------------------------------------------------------------------
...
import { 
  ...
  Package, Search, AlertCircle, Download, Users, CheckSquare, Square
} from 'lucide-react';
import { 
...
const PERSONEL_LISTESI = [
  "AHMET KALKAN", "HAMZA ATTAR", "UMUT SOYTAŞ", "AYKUT TASLI", 
  "ÜNAL DEMİR", "VOLKAN ŞAHAN", "MÜCAHİT ARSLAN", "NİHAN UĞUR", 
  "NAİL TAYLAK", "BASİL HAVATİMİ", "VEYSEL ÇARKLI", "KERİM AKDOĞAN"
];
...
export default function AnalizSayfasi() {
  ...
  const [performansAcik, setPerformansAcik] = useState(false); // 🔥 YENİ: Performans Modalı
  ...
  const [yeniKayit, setYeniKayit] = useState({
    ...
    price: '', technician: '' // 🔥 Burası artık otomatik dolacak
  });
  ...
  // ==========================================================================
  // 4. PERFORMANS ANALİZİ (YENİ FONKSİYON) 🔥
  // ==========================================================================
  const personelAnaliziYap = () => {
      ...
      // Sadece bu aya ait kayıtları al
      const buAykiKayitlar = kayitlar.filter(k => {
          const d = new Date(k.service_date);
          return d.getMonth() === buAy && d.getFullYear() === buYil;
      });

      return PERSONEL_LISTESI.map(personel => {
          // Personelin adının geçtiği işleri bul (Ahmet - Mehmet yapısında Ahmet'i bulur)
          const gittigiIsler = buAykiKayitlar.filter(k => k.technician && k.technician.includes(personel));
          return {
              ad: personel,
  ...
  const duzenle = (e: any, kayit: any) => {
      ...
      // Mevcut teknisyenleri string'den array'e çevir (checkboxlar için)
  ...
  const kaydetVeyaGuncelle = async () => {
    ...
    let error;
    if (duzenlemeId) error = (await supabase.from('completed_services').update(veriPaketi).eq('id', duzenlemeId)).error;
    else error = (await supabase.from('completed_services').insert([veriPaketi])).error;
    
    if (error) alert("Hata: " + error.message);
    else { alert(duzenlemeId ? "Güncellendi! ✅" : "Kaydedildi! ✅"); setFormAcik(false); formuSifirla(); tumVerileriGetir(); }
    setYukleniyor(false);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-6 pb-24 relative font-sans">
      ...
      {/* =========================================================================
          🔥 MODAL: FORM (YENİ EKLEME VE DÜZENLEME PENCERESİ) - GÜNCELLENDİ 🔥
          ========================================================================= */}
      <AnimatePresence>
        {formAcik && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[9999] flex items-center justify-center p-4" onClick={() => setFormAcik(false)}>
                <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} ref={modalRef} exit={{ scale: 0.95, opacity: 0, y: 20 }} className="bg-white w-full max-w-3xl rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="p-5 border-t border-slate-100 bg-slate-50 flex gap-3 shrink-0">
                        ...
                        <button onClick={kaydetVeyaGuncelle} className={`flex-[2] py-3 text-white font-bold rounded-xl hover:shadow-lg transition ${duzenlemeId ? 'bg-orange-600 hover:bg-orange-700' : 'bg-blue-600 hover:bg-blue-700'}`}>{duzenlemeId ? 'Kaydı Güncelle' : 'Kaydet ve Tamamla'}</button>
                    </div>
                </motion.div>
            </motion.div>
        )}
      ...
      {/* =========================================================================
          🔥 YENİ MODAL: EKİP PERFORMANS ANALİZİ 🔥
          ========================================================================= */}
      <AnimatePresence>
        {performansAcik && (
                    ...
                    <div className="p-6 bg-purple-50 border-b border-purple-100 flex justify-between items-center">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs font-bold text-purple-600 uppercase tracking-widest">{new Date().toLocaleString('tr-TR', { month: 'long', year: 'numeric' }).toUpperCase()} AYI ANALİZİ</p>
                            </div>
                        </div>
                                ...
                                <div className="space-y-6">
                                    <div className="flex justify-between items-end border-b pb-4">
                                        <div>
                                            ...
                                            <p className="text-sm text-slate-500">Bu ay tamamladığı servis ve montajlar.</p>
                                        </div>
                                    ...
                                    {aktifPersonelDetay.detaylar.length > 0 ? (
                                        ...
                                        <div className="text-center py-20 text-slate-400">
                                            ...
                                            <p>Bu ay henüz bir kaydı bulunmuyor.</p>
                                        </div>
                                    )}
                                </div>
                            ) : (
